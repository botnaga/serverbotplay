...
gaya1 = setting.gaya1
gaya2 = setting.gaya2
const fakeimage = fs.readFileSync ('./media/peler.jpg')
const thumb = fs.readFileSync ('./media/komtll.jpg')

//══════════[ Developer Number ]══════════//

developerNo = '6285892640252'
developerName= 'DEVELOPER PLAYSHOP'
ryukey = 'RyuBotz'
...
module.exports = marubotz = async (marubotz, mek, _welkom) => {
	try {
		...
		const botNumber = marubotz.user.jid
		const ownerNumber = [`${owner}@s.whatsapp.net`, `62858157008614@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? marubotz.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const groupMetadata = isGroup ? await marubotz.groupMetadata(from) : ''.toString
        ...
        m = simple.smsg(marubotz, mek)
        var pes = (type === 'conversation' && mek.message.conversation) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text ? mek.message.extendedTextMessage.text : ''
        const messagesD = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
		const conts = mek.key.fromMe ? marubotz.user.jid : marubotz.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? marubotz.user.name : conts.notify || conts.vname || conts.name || '-'    
		...
		let bio_nya = await marubotz.getStatus(sender)
		try {
			...
			try {
		pporang = await marubotz.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
...
marubotz.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
marubotz.sendMessage(from, hasil, type, options).catch(e => {
marubotz.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
      ...
      download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            marubotz.sendMessage(to, media, MessageType.sticker, { quoted: mek });
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        marubotz.sendMessage(from, media, MessageType.sticker, {quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
...
marubotz.sendMessage(from, buttonMessages, buttonsMessage, {
quoted: mek
})
}
...
marubotz.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await marubotz.prepareMessage(from, kma, image)
...
marubotz.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await marubotz.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await marubotz.prepareMessage(from, kma, location)
const buttonMessages = {
...
let po = marubotz.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "𝗠𝗘𝗡𝗨","footerText": `${tanggal}`,"listType": "SINGLE_SELECT","sections": list}}, {})
return marubotz.relayWAMessage(po, {waitForAck: true})
}
const reply = (teks) => {
marubotz.sendMessage(from, teks, text, {quoted:mek})
}
const sendMess = (hehe, teks) => {
marubotz.sendMessage(hehe, teks, text)
}
const isUrl = (url) => {
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%.+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%+.~#?&/=]*)/, 'gi'))
        }
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ? marubotz.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : marubotz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
}
const costum = (pesan, tipe, target, target2) => {
marubotz.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
}
...
const textImg = (teks) => { return marubotz.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/komtll.jpg')})}
const fakeitem = (teks) => { marubotz.sendMessage(from, teks, text, { quoted: { key:{ fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? { remoteJid :"6289523258649-1604595598@g.us" }: {})},message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync("./media/komtll.jpg"),"itemCount":9999999999,"status":"INQUIRY","surface":"CATALOG","message": `${ucapanWaktu} ${pushname}`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}},contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true})}
...
marubotz.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
let anugroupsend = await marubotz.groupMetadata(from)
let members = anugroupsend.participants
let ane = []
for (let i of members){
ane.push(i.jid)
}
marubotz.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/peler.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}
const hideTagKontak = async function(from, nomor, nama){
let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
let anuvcardoke = await marubotz.groupMetadata(from)
let members = anuvcardoke.participants
let ane = []
for (let i of members){
ane.push(i.jid)
}
marubotz.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
}
const sendKontak = (from, nomor, nama) => {
const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer ${botname}\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
marubotz.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
}
...
for (let anji of setik){
				if (budy === anji){
					...
					marubotz.sendMessage(from, result, sticker, { quoted: mek})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					marubotz.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					marubotz.sendMessage(from, result, image, { quoted: mek})
					}
				  }
...
╰───「 *LEVEL UP* 」`)
          ...
          sendButton(from, lvlup, '*Kontlo Bot*', but)
...
setTimeout(() => {
marubotz.groupRemove(from, [kic]).catch((e) => { reply(`BOTS MUST BE ADMIN`) })
}, 0)
}
...
marubotz.groupRemove(from, [sender])
}     

//══════════[ Dll ]══════════//

if (autoread){
marubotz.chatRead(from, "read")
} else if (autoketik) {
marubotz.updatePresence(from, Presence.composing)
} else if (autovn) {
marubotz.updatePresence(from, Presence.recording)
}
...
buttonText: 'MENU',
footerText: `*${botname}*`,
description: `Halo @${senderr.split('@')[0]}, Pilih menunya disini`,
sections: [
{
"title": `${tanggal} - ${time}`,
rows: [
...
{
"title": "[📚] IMAGE EFECT",
"description": `Display A List Of Features To images efect`,
...
marubotz.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [senderr]},quoted:mek})
break
case 'allmenu':
...
`
var mek = await getBuffer(`https://i.ibb.co/L5CjLrd/IMG-20220320-WA0017.jpg`)
marubotz.sendMessage(from, { contentText: `${menu}`, footerText: `*_${tanggal}_*`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'BACK ⬅️' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER 👤' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: mek, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'makermenu':
...
${gaya2} ${prefix}tebakkata
${gaya2} ${prefix}tebaklirik
${gaya2} ${prefix}tebakjenaka
${gaya2} ${prefix}tebakkimia
${gaya2} ${prefix}tebakbendera
`
var mek = await getBuffer(`https://i.ibb.co/L5CjLrd/IMG-20220320-WA0017.jpg`)
marubotz.sendMessage(from, { contentText: `${menu}`, footerText: `*_${tanggal}_*`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'BACK ⬅️' }, type: 1 },{ buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER 👤' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: mek, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
...
`
var mek = await getBuffer(`https://i.ibb.co/L5CjLrd/IMG-20220320-WA0017.jpg`)
marubotz.sendMessage(from, { contentText: `${menu}`, footerText: `*_${tanggal}_*`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'BACK ⬅️' }, type: 1 },{ buttonId: `${prefix}abcde`, buttonText: { displayText: '\n\nAKU ADALAH GAY' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: mek, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'asupanmenu':
...
gambar = fs.readFileSync('./media/peli.jpg')
menunya = `*「 PAYMENT 」*

• qris : Seken Qris Di Atas
• Gopay: _085892640252_`
but = [
...
Please Choose Whether Document, Audio or Video Below`
        sendButMessage(from, result, `*Komtlo Bot*`, [
          {
...
case 'spotify':{
    ...
    await marubotz.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
    get_audio = await getBuffer(get_result.link)
    await marubotz.sendMessage(from, get_audio, audio, { mimetype: 'audio/mpeg', filename: `${get_result.title}.mp3`, quoted: mek})
    }
    break
   case 'soundcloud':
				...
				zee.SoundCloud(`${q}`).then(async (data) => {
                    ...
                    marubotz.sendMessage(from , await getBuffer(data.medias[0].url), audio,{ quoted: mek, mimetype: 'audio/mp4' })
				})
			break
	case 'telesticker': case 'telegramsticker': case 'tstiker': {
			...
			marubotz.sendMessage(from, await getBuffer(telestc[i].url), sticker, {mimetype:'image/webp',quoted: mek})
			}
		}
		break
case 'tiktoknowm':
   case 'tiktok':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					marubotz.sendMessage(from,noweem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
...
case 'tiktokwm':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					marubotz.sendMessage(from,weem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
				}).catch((err) => reply(`Invalid link`))
 ...
 case 'tiktokmp3':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					marubotz.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mp4',filename: `Tiktok Music.mp3`,quoted: mek})
					})
				}).catch((err) => reply(`Invalid link`))
...
case 'play':
		 ...
		 if (!arg && !m.quoted) return reply('Masukan judul atau Reply Judul Lagunya')
			...
			const captions = `*Data Berhasil Didapatkan!*\n\n*Title : ${title}*\n*Id : ${id}*\n*Channel : ${playy.data.result[0].channel.name}*\n*Views : ${playy.data.result[0].views}*\n*Diupload : ${playy.data.result[0].uploaded}*\n*Duration : ${playy.data.result[0].duration}*\n*Link : ${link}*\n\n_Silahkan pilih jenis file di bawah, get vedeo / audio_`
                                    gambar = await getBuffer(thumbnail)
gbutsan = [
  {buttonId: `${prefix}ytmp4 ${link}`, buttonText: {displayText: 'VIDEO'}, type: 'RESPONSE'},
  {buttonId: `${prefix}ytmp3 ${link}`, buttonText: {displayText: 'AUDIO'}, type: 'RESPONSE'}
]
tes = await marubotz.prepareMessage('0@s.whatsapp.net', {jpegThumbnail: gambar}, location, {thumbnail: gambar})
 gbuttonan = {
locationMessage: tes.message.locationMessage,
    contentText: captions,
    footerText: '© kontlobot',
    buttons: gbutsan,
    headerType: 'LOCATION'
}
		marubotz.sendMessage(from, gbuttonan, MessageType.buttonsMessage, {quoted: mek})
			} else if (!arg) {
...
locationMessage: tes.message.locationMessage,
    ...
    headerType: 'LOCATION'
        ...
        break
case 'ytmp4':
if (!q) return reply(`Kirim perintah *${prefix}ytmp4 [linkYt]*`)
                ...
                try {
                    ...
                    .then((res) => {
                        ...
                        .then((a) => {
                            if (Number(filesize) >= 40000) return sendMediaURL(from, thumb, `┏┉⌣ ┈̥-̶̯͡..̷̴✽̶┄┈┈┈┈┈┈┈┈┈┈┉┓
┆ *YOUTUBE MP4*
└┈┈┈┈┈┈┈┈┈┈┈⌣ ┈̥-̶̯͡..̷̴✽̶⌣ ✽̶
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                            sendMediaURL(from, thumb, captionsYtmp4, mek)
                            sendMediaURL(from, dl_link, '', mek)
...
break
case 'ytmp3':
if (!q) return reply(`Kirim perintah *${prefix}ytmp3 [linkYt]*`)
                if (!isUrl(args[0]) && !args[0].includes('youtube.com')) return reply(mess.error.Iv)
                try {
                    reply(mess.wait)
                    yta(q)
...
_Silahkan tunggu file media sedang dikirim mungkin butuh beberapa menit_`
                            sendMediaURL(from, thumb, captions, mek)
                            sendMediaURL(from, dl_link, '', mek)
                           // limitAdd(sender, limit)
                        })
                    })
...
--> muek*/
                ...
                reply(mess.wait)
                    getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${Lolhumanbykur}`).then((gambar) => {
                        marubotz.sendMessage(from, gambar, image, { quoted: mek })
                    })
                    break
                   case 'fox':  
                   anufox= await fetchJson(`https://some-random-api.ml/img/fox`)
                   anu1 = await getBuffer(anufox.link)
                   marubotz.sendMessage(from, anu1, image, {caption: `Here you go!`, quoted: mek})
                   break
                   case 'dog':  
...
owgi = await marubotz.downloadMediaMessage(boij)
res = await uploadImages(owgi)
reply(mess.wait)
buff = await getBuffer(`https://api.zekais.com/${command}?url=${res}&apikey=WtmS9r3o`)
marubotz.sendMessage(from, buff, image, {quoted: ftrol, caption: `Succes By : ${ownername}`})
} else {
...
break
             case 'sao':
				...
				marubotz.updatePresence(from, Presence.composing) 
                 ...
                 marubotz.sendMessage(from, swordartonline, image, {quoted: mek, caption: 'swort art online\nDogeBot'})
				 break
				case 'hsdxd':
				reply(mess.wait)
				 marubotz.updatePresence(from, Presence.composing) 
                 ...
                 marubotz.sendMessage(from, highschooldxd, image, {quoted: mek, caption: 'Here u go!'})
				break
				 case 'lovelive':
				reply(mess.wait)
				 marubotz.updatePresence(from, Presence.composing) 
                 ...
                 marubotz.sendMessage(from, lovelive, image, {quoted: mek, caption: 'Here u go!'})
				break
...
sendButVideo(from, pll, `*Komtlo*`, get_result, [      
{
buttonId: `${prefix+command}`,
buttonText: {
displayText: `NEXT ➡️`,
...
//----> 1 TEXT <----//
                ...
                case 'freefire':          
                    if (args.length == 0) return reply(`Example: ${prefix + command} muek`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${lolkey}&text=${ini_txt}`).then((gambar) => {
                        marubotz.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Here you go!`, quoted : mek}) 
                    })
                    break
case 'blackpink':
case 'halloween':
...
marubotz.sendMessage(from, oke, image, {quoted: mek, caption: 'Here u go!😛'})
break
case 'hartatahta': 
...
marubotz.sendMessage(from, harta, image, {quoted: mek, caption: 'Here u go!😛'})
break
...
marubotz.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :'+clue+'\nWaktu : 30s' }).then(() => {
...
jawaban = anu.jawaban
pertanyaan = anu.soal
marubotz.sendMessage(from, '```Tebak Kata```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
...
marubotz.sendMessage(from, '```Tebak Lirik```\n\n• *Soal* :'+anu.result.question+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
...
marubotz.sendMessage(from, '```Tebak Jenaka```\n\n• *Soal* :'+anu.result.pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
...
marubotz.sendMessage(from, '```Tebak Kimia```\n\n• *Soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
...
marubotz.sendMessage(from, '```Tebak Bendera```\n\n• *Bendera* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
...
marubotz.sendMessage(from, '```Caklontong```\n\n• *soal* :'+pertanyaan+'\n• *Waktu :* 30s', text, { quoted: mek}).then(() => {
...
marubotz.sendMessage(from, pornhub, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'pornhub':
...
marubotz.sendMessage(from, girlneko, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'sadboy':
...
marubotz.sendMessage(from, girlneko2, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'makerkaneki':
...
marubotz.sendMessage(from, girlneko8383, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'rem':
...
marubotz.sendMessage(from, girlneko11111, image, {caption: `Here u go!??`, quoted: mek})
break
case 'lolimaker':
...
marubotz.sendMessage(from, girlnekojkjk, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'gura':
...
marubotz.sendMessage(from, girlnekoooo, image, {caption: `Here u go!😛`, quoted: mek})
break
                         case 'wolf3':
                   ...
                   marubotz.sendMessage(from, anuwolf3, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it?`, quoted: mek})
                   break
                          case 't3d':
                   ...
                   marubotz.sendMessage(from, anut3d, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'logoa':
       
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} muek&lol*`)
                   var F = body.slice(7)
				   var F1 = F.split("|")[0];
				   var F2 = F.split("|")[1]; 
                   reply(mess.wait)
                   anulogoa = await getBuffer(`${ApiZeks}/api/logoaveng?text1=${F1}&text2=${F2}&apikey=${zeksApikey}`)
                   marubotz.sendMessage(from, anulogoa, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'phlogo':  
                   ...
                   marubotz.sendMessage(from, anuphlogo, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'marvel3':
                   ...
                   marubotz.sendMessage(from, anumrvl3, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'leavest':  
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} muek*`)
                   F = body.slice(9)
                   ...
                   marubotz.sendMessage(from, anulvst, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break                   
       case 'notewrite':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} muek*`)
                   F = body.slice(7)
                   ...
                   marubotz.sendMessage(from, anunw, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'neon2':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} muek*`)
                   F = body.slice(7)
                   ...
                   marubotz.sendMessage(from, anunion2, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break 
...
+ 'END:VCARD'.trim()
marubotz.sendMessage(from, {displayName: `The owner ${botname}`, vcard: vcard2}, contact, 
{ quoted: fgi, 
})
reply(`_The above contact is my owner_`)
break
case 'developer':
...
marubotz.sendMessage(from, devsound, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
break
case 'runtime':
...
marubotz.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
break
...
await marubotz.sendMessage(from, ini_buffer, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
break
...
marubotz.sendMessage(from, buff, image, {quoted: mek, caption : teks})
break
...
//══════════[ CONVERT FEATURES ]══════════//
      ...
      case 'squirrel':
        ...
        media = await marubotz.downloadAndSaveMediaMessage(encmedia012)
        ran = getRandom('.mp3')
        exec(`ffmpeg -i ${media} -af atempo=1/2,asetrate=44500*2/1 ${ran}`, (err, stderr, stdout) => {
          fs.unlinkSync(media)
          if (err) return reply('Error!')
          hah = fs.readFileSync(ran)
          marubotz.sendMessage(from, hah, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
          fs.unlinkSync(ran)
        })
        break
      case 'blub':
        ...
        media = await marubotz.downloadAndSaveMediaMessage(encmediakekek)
        ran = getRandom('.mp3')
      ...
      case 'ghost':
        ...
        media = await marubotz.downloadAndSaveMediaMessage(encmedia777)
        ran = getRandom('.mp3')
      ...
      case 'cutesound':
        ...
        media = await marubotz.downloadAndSaveMediaMessage(encmedia100)
        ran = getRandom('.mp3')
        exec(`ffmpeg -i ${media} -filter:a "atempo=1.0,asetrate=50000" ${ran}`, (err, stderr, stdout) => {
          fs.unlinkSync(media)
          if (err) return reply('Error!')
          riu = fs.readFileSync(ran)
          marubotz.sendMessage(from, riu, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
          fs.unlinkSync(ran)
        })
        break
case 'slowmo': case 'slow':{
								try {
										...
										media = await marubotz.downloadAndSaveMediaMessage(encmedia22)
										ran = getRandom('.mp3')
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
											fs.unlinkSync(media)
											if (err) return reply('Error!')
											uhh = fs.readFileSync(ran)
											marubotz.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
											fs.unlinkSync(ran)
											})
											} catch (e) {
												reply('Error!')
...
case 'fast':{
									try {
										...
										media = await marubotz.downloadAndSaveMediaMessage(encmedia11)
										ran = getRandom('.mp3')
...
case 'vibra': case 'vibrato':{
									...
									media = await marubotz.downloadAndSaveMediaMessage(encmedia33)
									ran = getRandom('.mp3')
    ...
    case 'nightcore':            
		  ...
		  core = await marubotz.downloadAndSaveMediaMessage(night)
		  ran = getRandom('.mp3')
		  reply(mess.wait)
		  ...
		  if (err) return reply('Error!')
		  hah = fs.readFileSync(ran)
		  marubotz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:false, quoted: mek, ptt: true})
		  fs.unlinkSync(ran)
		  })
	      break
 case 'vnsec':
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await marubotz.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						marubotz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
						fs.unlinkSync(mediam)
				break
				case 'vidsec':
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					median = await marubotz.downloadAndSaveMediaMessage(encmedian)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(median)
						marubotz.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
						fs.unlinkSync(median)
				break
 case 'robot':
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await marubotz.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
marubotz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'fat':
					encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediaz = await marubotz.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					marubotz.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'reverse':
	encmediau = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	mediau = await marubotz.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
					...
					mediao = await marubotz.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
 ...
 case 'tomp3':
					...
					mediad = await marubotz.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						marubotz.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
 case 'toimg':
...
media = await marubotz.downloadAndSaveMediaMessage(encmediatoimg)
ran = getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('Failed, when converting sticker to image')
buffer = fs.readFileSync(ran)
marubotz.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
fs.unlinkSync(ran)
})
break
case 'tts':
        ...
        if (args.length < 2) return marubotz.sendMessage(from, 'Where is the text bro??', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                marubotz.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
//══════════[ LEVELING FEATURES ]══════════//
	...
	case 'level':
					...
					marubotz.sendMessage(from, resul, text, { quoted: mek})
					.catch(async (err) => {
					console.error(err)
					await reply(`Error!\n${err}`)
					})
					...
					case 'profile':
					if (!isGroup) return reply(mess.group)
				    let anuprofileoke = await marubotz.groupMetadata(from)
                    const thu = await marubotz.getStatus(anuprofileoke.participants[0], MessageType.text)
    				marubotz.updatePresence(from, Presence.composing)
    				try {
					ppimg = await marubotz.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					} catch {
					...
					marubotz.sendMessage(from, buffer, image, {quoted: mek, caption: profile})
					break
...
case 'ttp4':  
				...
				case 'attp':
					...
					marubotz.sendMessage(from, atetepe12, sticker, { quoted: mek })
					break
case 'memegenerator': case 'memegen':{
									...
									try {
									   ...
									   var mediiia = await marubotz.downloadMediaMessage(enmediaokekek)
										...
										marubotz.sendMessage(from, resu, image, {caption:'.stikerin bang', thumbnail: Buffer.alloc(0), quoted: mek})
 	...
 	case 'stickermeme': case 'memesticker': case 'memestick': case 'stickmeme': case 'stcmeme': case 'smeme':{
						if (args.length < 1) return reply(`Send orders *${prefix + command}* DogeBot`)
									if (q.includes('|')) return reply(`Reply to an image with a caption, For Example *${prefix + command}* muek`)
									try {
										if (!isQuotedImage) return reply(`Reply to an image!`)
										reply(mess.wait)
										var teks2 = args.join(' ')
										var enmedialel = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
										var mediia = await marubotz.downloadMediaMessage(enmedialel)
...
case 'takesticker':
					case 'take':
						...
						const media2 = await marubotz.downloadAndSaveMediaMessage(encmediaoo1, `./sticker/${sender}`)
						const packname = ppp.split('|')[0]
						const author = ppp.split('|')[1]
						exif.create(packname, author, `takestick_${sender}`)
						exec(`webpmux -set exif ./sticker/takestick_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
							if (error) return reply(mess.error.api)
							marubotz.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
...
case 'stickerwm':
					case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await marubotz.downloadAndSaveMediaMessage(encmedia9191, `./sticker/${sender}`)
							...
							const media = await marubotz.downloadAndSaveMediaMessage(encmediaokekak, `./sticker/${sender}`)
							...
							sticWait(from)
								await ffmpeg(`${media}`)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/stickwm_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											if (error) return reply(mess.error.api)
											marubotz.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: ftex})
											fs.unlinkSync(media)
											fs.unlinkSync(`./sticker/${sender}.webp`)
											fs.unlinkSync(`./sticker/stickwm_${sender}.exif`)
										})
...
const media = await marubotz.downloadAndSaveMediaMessage(encmedialoli)
ran = '666.webp'
await ffmpeg(`./${media}`)
.input(media)
.on('start', function (cmd) {
...
.on('end', function () {
console.log('Finish')
marubotz.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
.addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
...
const media = await marubotz.downloadAndSaveMediaMessage(encmedialoli22)
ran = '999.webp'
reply(mess.wait)
await ffmpeg(`./${media}`)
...
var media = await marubotz.downloadAndSaveMediaMessage(enmediahe1)
var njay = await imgbb('520bd6f6209077d1777c2a4f20c509c2', media)
var resu = await getBuffer(`https://api.memegen.link/images/custom/${teks1}/${teks2}.png?background=${njay.display_url}`)
marubotz.sendMessage(from, resu, image, {quoted: mek})
fs.unlinkSync(media)
} catch (e) {
return reply(`${e}`)
console.log(e)
      ...
      if (!isGroup) return reply(mess.only.group)
        ...
        break
                  ...
                  sange = body.slice(1)
					...
					marubotz.sendMessage(from, 'Question : *'+sange+'*\n\nAnswer: '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycheck':
					// source code by muek⛔
                  gayy = body.slice(1)
					...
					marubotz.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
                case 'lesbiancheck':
					// source code by muek⛔
                  lesbii = body.slice(1)
					...
					marubotz.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by muek⛔
ganteng = body.slice(1)
					...
					marubotz.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by muek⛔
cantik = body.slice(1)
					...
					marubotz.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'charactercheck':
					// source code by muek⛔
watak = body.slice(1)
					...
					marubotz.sendMessage(from, 'Question : *'+watak+'*\n\nAnswer : '+ tak, text, { quoted: mek })
				        break
...
//══════════[ GAME FEATURES ]══════════//
    ...
    "say i love the bot owner muek through voice note",
    ...
    "breakup with your best friend for 5hrs without telling him/her that its a dare",
     ...
     "send abusive words in any grup, excepting this grup, and send screenshot proof here"
              ...
              marubotz.sendMessage(from, buffer, image, { quoted: mek, caption: '_*DARE*_\n'+ der })
              break
                            break
       case 'truth':
              const truth =[
    ...
    "do you love the bot creator, muek?ðŸ¦„",
    ...
    "do you play pubg, if you then send ur id number"
              ...
              buffer = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
              marubotz.sendMessage(from, buffer, image, { quoted: mek, caption: '_*TRUTH*_\n'+ trut })
              break
...
case 'bc':
             ...
             if (args.length < 1) return reply('Where is the text?')
             anu100 = await marubotz.chats.all()
             if (isMedia && !marubotz.message.videoMessage || isQuotedImage) {
             const encmediaboomb = isQuotedImage ? JSON.parse(JSON.stringify(marubotz).replace('quotedM','m')).message.extendedTextMessage.contextInfo : marubotz
             bc100 = await marubotz.downloadMediaMessage(encmediaboomb)
             for (let _ of anu100) {
             marubotz.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 Komtlo Broadcast 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu100) {
             marubotz.sendMessage(_.jid, 
			{"contentText": `*「 KOMTLO BROADCASTING MESSAGE 」*\n\n${body.slice(4)}`,
			"footerText": `${tanggal}`,
			"buttons": [
...
anubc2 = await marubotz.chats.all()
if (isMedia && !mek.message.videoMessage || isQuotedImage) {
...
buff = await marubotz.downloadMediaMessage(encmedia12345)
for (let _ of anubc2) {
marubotz.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
}
...
buff = await marubotz.downloadMediaMessage(encmediaki)
for (let _ of anubc2) {
marubotz.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
}
reply(`Broadcast success ${body.slice(4)}`)
} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
...
buff = await marubotz.downloadMediaMessage(encmediadirk)
for (let _ of anubc2) {
marubotz.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
}
reply(`Broadcast success ${body.slice(4)}`)
} else {
for (let _ of anubc2) {
...
bcgc = await marubotz.downloadMediaMessage(encmediabcgc)
for (let _ of groupMembers) {
marubotz.sendMessage(_.jid, bcgc, image, { caption: `*「 KOMTLO BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
sendMess(_.jid, `*「 KOMTLO BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
...
if (!isOwner && !mek.key.fromMe) return  reply(mess.only.owner)
anuclearall = await marubotz.chats.all()
marubotz.setMaxListeners(25)
for (let _ of anuclearall) {
marubotz.deleteChat(_.jid)
}
...
media = await marubotz.downloadAndSaveMediaMessage(enmediaheheh, './database/sampah/media_user')
await marubotz.updateProfilePicture(botNumber, media)
...
marubotz.updateProfileName(anuoke112)
...
if (!Number(oi2)) return reply('The number must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  marubotz.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Max 50!')
	  ...
	  marubotz.sendMessage(from, teks, MessageType.text)
	  }
	...
	if (Number(args[0]) >= 50) return reply('Max 50!')
	  for (let i = 0; i < args[0]; i++) {
	  marubotz.sendMessage(from, anutospem, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await marubotz.downloadAndSaveMediaMessage(encmediat)
				anutospem2 = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Max 50!')
	  for (let i = 0; i < args[0]; i++) {
	  marubotz.sendMessage(from, anu2spem2, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await marubotz.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Max 50!')
	if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  marubotz.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'spam':
				...
				for (let i = 0; i < argzi[1]; i++){
					marubotz.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				      case 'leaveall':
             if (!isOwner) return  reply(mess.only.owner)
             let totalgroup = marubotz.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
             for (let id of totalgroup) {
             sendMess(id, 'byee!', null)
             marubotz.groupLeave(id)
}
             break
             case 'addvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					...
					marubotz.sendMessage(from, `Success, please check with *${prefix}vnlist*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					try {
				...
				case 'listvn':
					...
					marubotz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
					case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					...
					marubotz.sendMessage(from, `Success, please check with *${prefix}imglist*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					try {
				...
				case 'listimage':
					...
					marubotz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
					case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					if (!isQuotedSticker) return reply('Reply sticker')
					...
					marubotz.sendMessage(from, `Success, please check with *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					try {
				...
				case 'liststicker':
					...
					marubotz.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					break
...
//══════════[ GROUP FEATURES ]══════════//
			...
			if (!isGroup) return reply(mess.only.group)
				...
				if (mek.message.extendedTextMessage != undefined){
					...
					marubotz.groupCreate(argz[0], anucgc)
					reply(`Success in creating a group ${argz[0]}`)
                }
				break
case 'getbio':
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await marubotz.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
case 'getdp':
            ...
            pictt = await marubotz.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            marubotz.sendMessage(from, pict, image, {quoted: mek})
            break
                    case 'getname':
        var ambl = mek.message.extendedTextMessage.contextInfo.participant
const sname = marubotz.contacts[ambl] != undefined ? marubotz.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : marubotz.contacts[ambl].notify || marubotz.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
case 'leave':
            if (!isGroup) return reply(mess.only.group)
                    if (isGroupAdmins || isOwner) {
                    	marubotz.groupLeave(from)
                    } else {
...
case 'getdescgc':
				...
				if (!isGroup) return reply(mess.only.group)
					anugetdescgc = from
			   metadete = await marubotz.groupMetadata(anugetdescgc)
				marubotz.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
case 'nsfw':
...
await marubotz.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: mek})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
                   ...
                   reply(`Successful opening group ${groupName}`)
						marubotz.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply(mess.only.group)
                   ...
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
						...
						marubotz.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
case 'grouplink':
case 'gruplink':
case 'gclink':
...
if (!isGroup) return reply(mess.only.group)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
linkgc = await marubotz.groupInviteCode(from)
yeh = `https://chat.whatsapp.com/${linkgc}\n\n*${groupName}* group link`
marubotz.sendMessage(from, yeh, text, { quoted: fgi })
break
case 'promote' :
...
marubotz.groupMakeAdmin(from, mentioned)
} else {
...
mentions(teks, mentioned, true)
marubotz.groupDemoteAdmin(from, mentioned)
} else {
...
marubotz.groupDemoteAdmin(from, members_id)
break
case 'promoteall':
...
marubotz.groupMakeAdmin(from, members_id)
break
case 'add' :
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid;
if (mentioned.length > 1) {
marubotz.groupRemove(from, mentioned);
reply(mess.success);
} else if (mentioned.length < 1) {
anukick = mek.message.extendedTextMessage.contextInfo.participant;
marubotz.groupRemove(from, [anukick]);
reply(mess.success);
} else {
marubotz.groupRemove(from, mentioned);
reply(mess.success);
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
marubotz.groupUpdateSubject(from, `${body.slice(9)}`)
marubotz.sendMessage(from, `\`\`\`Success ✅, Renamed the group to\`\`\` *${body.slice(9)}*`, text, { quoted: mek })
break
case 'setdesc':
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
marubotz.groupUpdateDescription(from, `${body.slice(9)}`)
marubotz.sendMessage(from, `\`\`\`Success ✅, Changing group description\`\`\` *${groupMetadata.subject}* Became: *${body.slice(9)}*`, text, { quoted: fgi })
break
case 'setgrouppp':
case 'setgruppp':
case 'setpp':
...
let media = await marubotz.downloadMediaMessage(encmediasetppgc)
marubotz.updateProfilePicture(from, media)
...
if (!isGroup) return reply(mess.only.group)
try {
var pic = await marubotz.getProfilePicture(from)
} catch {
var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*Group ID :* ${from}\n*Made :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Kolkata').format('DD/MM/YYYY HH:mm:ss')}\n*Group Owner :* @${groupMetadata.owner.split('@')[0]}\n*Number of Admins :* ${groupAdmins.length}\n*Number of participants :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n\n${groupMetadata.desc}`
marubotz.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
break
case 'resetlinkgc':
case 'resetlinkgroup':
case 'resetlinkgrup':
...
marubotz.query({json, expect200: true})
...
break
       ...
       case 'here':                
             ...
             let online = [...Object.keys(marubotz.chats.get(ido).presences), marubotz.user.jid]
             marubotz.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
...
case 'sticktag':
            ...
            file = await marubotz.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await marubotz.groupMetadata(from)
            ...
            marubotz.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            marubotz.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            marubotz.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            marubotz.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            marubotz.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await marubotz.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
...
marubotz.sendMessage(from, options, text, { quoted: mek })
break
case 'contag':
...
reply(`${err}`)
	...
	marubotz.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Dream Guy muek",body:"muek",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/peler.jpg'),sourceUrl:"https://wa.me/6285892640252"}}})
        }
	// console.log(e)
	}
}
